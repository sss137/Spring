...
public class BbsServiceImpl implements BbsService {
  ...
  public boolean addBbs(BbsDTO parent) {  //파라미터 객체 parent에는 content만 존재합니다.
    ...
    try {
      int addedBbsCount = bbsDAO.insertParentBbs(parent); //전달한 parent에는 bbsId가 null이지만 insert 이후 PK값을 가지게 됩니다.
      if(addedBbsCount != 1) {
        throw new RuntimeException("게시글 등록 실패");
      }
      
      int updatedBbsCount = bbsDAO.updateGroupId(parent); //전달한 parent에는 bbsId와 content가 존재합니다.
